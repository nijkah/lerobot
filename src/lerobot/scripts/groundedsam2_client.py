...
"""Utility client to query a Grounded SAM 2 inference server."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--server-url",
        required=True,
        help="Base URL of the Grounded SAM 2 server (e.g., http://gpu-node:8090).",
    )
    parser.add_argument("--caption", required=True, help="Text prompt describing target objects.")
    parser.add_argument("--image-path", help="Path to a single image file to send.")
    ...
    parser.add_argument("--min-box-area", type=float, default=1_500.0, help="Minimum bbox area in pixels.")
    ...
    parser.add_argument("--mask-alpha", type=float, default=0.35, help="Alpha blend factor for masks (0-1).")
    parser.add_argument("--save-json", help="Optional path to dump response detections as JSON.")
...
def _draw_detections(frame: np.ndarray, detections, mask_alpha: float) -> None:
    overlay = frame.copy()
    for det in detections:
        ...
        color = np.array([0, 165, 255], dtype=np.float32)
...
def _print_detections(detections) -> None:
    ...
    for idx, det in enumerate(detections):
        ...
        mask_status = "mask" if det.mask is not None else "no-mask"
        LOG.info("  %d) %s conf=%.3f bbox=[%s] (%s)", idx + 1, det.label, det.confidence, bbox, mask_status)
...
def main() -> None:
    ...
    if args.image_path:
        ...
        if args.show:
            vis = frame.copy()
            _draw_detections(vis, detections, args.mask_alpha)
            cv2.imshow("Grounded SAM 2 client", vis)
